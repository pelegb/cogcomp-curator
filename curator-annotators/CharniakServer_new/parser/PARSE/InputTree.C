...
InputTree(InputTree* it) :
  start_(it->start_), finish_(it->finish_), word_(it->word_),
    term_(it->term_), ntInfo_(it->ntInfo_), parent_(it->parent_),
     headTree_(headTree_)
{
  ConstInputTreesIter subti = it->subTrees().begin();
...
newParse(istream& is, int& strt, InputTree* par)
{
  ...
  if(Term::Language == "Ch")
      ...
      if(!ctrm)
	{
	  ...
	  if ( NULL == ctrm )
	    throw CharniakException( "null pointer (ctrm)." );
...
#ifdef PRINT_OFFSETS
      cerr << " start: " << startOffset_ << "; end: " << endOffset_ << endl;
#endif
  ...
  for(int j = 0 ; j < i ; j++)
    {
      ...
      if ( li == l.end() )
	throw CharniakException( "index out of range (li)." );
