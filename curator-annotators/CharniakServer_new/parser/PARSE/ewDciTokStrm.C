...
/*
ewDciTokStrm::
ewDciTokStrm( const ECString& name, const bool tokenize )
  :
  useCin(0),
  sentenceName(""),
  istr_(name.c_str()),
...
ewDciTokStrm( const ECString & name, const bool tokenize ): 
	ifstr_(name.c_str()), 
    	...
    	ellipFlag( 0 ),                   // counts how many dots are in an ellipsis
    	parenFlag( 0 ),              // ParenFlag = 0 except while words
    	currentCharOffset_( 0 ),       // index of current character in original string
    	tokenize_( tokenize )            // do or don't tokenize input
{}


ewDciTokStrm::
ewDciTokStrm( istream & strm, const bool tokenize ): 
	useCin(0),
    	istr_( strm ),
    	savedWrd_( "" ),             // holds not-yet-processed parts of current Wrd
    	nextWrd_( "" ),              // holds "on-deck" Wrd
...
//StringWithOffsets indead of ECString in this function

StringWithOffsets                            // Discard Wrds from input stream
ewDciTokStrm::                               //  until reaching the next <s> ;
flush_to_sentence()                          //  return the <s> . Return empty
{                                            //  string if stream runs dry.
    	//ECString	str;
	StringWithOffsets nextWord;
		...
		getNextWordFrom(( useCin ? cin : istr_ ), nextWord);
    		string str = nextWord.surfaceForm_;
		...
		if( str == "</DOC>") docEnd = 1;
			...
			//if(useCin) cin >> str;
	  		//else istr_ >> str;
			getNextWordFrom(( useCin ? cin : istr_ ), nextWord);
    			str = nextWord.surfaceForm_;	  		


			nextWord.surfaceForm_ = "<s>";
	  		return nextWord;
		}
...
StringWithOffsets
ewDciTokStrm::nextWrd2() {
  	StringWithOffsets str;
  	
 	if( !istr_) return str;
  
  	getNextWordFrom( istr_, str );
  	return str;
}
...
StringWithOffsets  	// Update savedWrd_ and nextWrd_; return next Wrd by call-
...
{
    	if (nextWrd_.surfaceForm_ == "</s>") {
		if (savedWrd_.surfaceForm_.length()) { // return saved word first
            		StringWithOffsets saved = StringWithOffsets(savedWrd_);	//TODO:problem with this constructor
            		savedWrd_ = StringWithOffsets();

            		return saved;
        	} else {
            		nextWrd_ = StringWithOffsets();
            		return StringWithOffsets("</s>", -1, -1);	//TODO:problem with this constructor
        	}
    	}

    	if( !savedWrd_.surfaceForm_.length() ) {
		savedWrd_ = nextWrd_;
		nextWrd_ = nextWrd2();
...
/*** Execution reaches this point iff. a new Wrd has just "come on deck." ***/
    	...
    	if( savedWrd_.surfaceForm_ == "<COMMENT>" )             	// Comments occur only as the
   	{                                           		//  last Wrds in a <s>...</s>
		nextWrd_ = flush_to_sentence();         	//  line.  Discard the comment
		savedWrd_ = StringWithOffsets();                //  itself and the "Paragraph-
		return StringWithOffsets("</s>", -1, -1);       //  ing Error" (unbracketed
    	}                                           		//  text) that follows.

    	if(docEnd) {
		docEnd = 0;
		return StringWithOffsets("</DOC>", -1, -1);
      	}
                                        			// remember that global savedWrd_ gets
                                                  		// reset (side effect) while;
    	StringWithOffsets retWrd = splitAtPunc( savedWrd_ );	// <-- this line executes!

	string retWrdStr = retWrd.surfaceForm_;
  	string savedWrdStr = savedWrd_.surfaceForm_;
  	string nextWrdStr = nextWrd_.surfaceForm_;


    	if( ellipFlag && retWrdStr != "." ) { ellipFlag = 0; }             	// update
    	if( retWrdStr == "." ) { ellipFlag++; }                            		// flags
    	if( nextWrdStr.length() && nextWrdStr[0] == '('                      		// before
		&& ( retWrdStr == "!" || retWrdStr == "?"                        		// return
	     	|| retWrdStr == "." && ellipFlag != 3 ))                  			//    .
    	{ parenFlag = 1; }                                               		//    .
                                                                     			//    .
    	if( retWrdStr == "(" )                                             		//    .
		if( !parenFlag ) { parenFlag = -1; }                         		//    .
		else if( !( Cap(savedWrdStr)                                   		//    .
		    	|| !savedWrdStr.length() && Cap(nextWrdStr) ))       		//    .
			{ parenFlag = 0; }                                           	//    .
                                                                     			//    .
    	if( retWrdStr == ")" || retWrdStr == "</s>" || retWrdStr == "</S>" ) { parenFlag = 0; }  //.....

    	escape_parens(retWrdStr);

    	// a hack so that '' comes out as closed quote;
    	if(savedWrdStr == "\'\'") savedWrdStr = "\"";

	retWrd.surfaceForm_ = retWrdStr;
	savedWrd_.surfaceForm_ = savedWrdStr;
...
StringWithOffsets                     // Extract and return the first Wrd that can be
ewDciTokStrm::                // split off the front end of savedWrd_; reset
splitAtPunc( StringWithOffsets term )      // savedWrd_ to hold everything that's left.
{  
  if ( !tokenize_ ) {
    savedWrd_ = StringWithOffsets();
    return term;
...
/***  These are punctuation characters that may be splitters.  The default
  ...
  string seq = term.surfaceForm_;
  int origStartOffset = term.startOffset_;
  int origEndOffset = term.endOffset_;
...
/***  Move the puncIndex forward to skip over puncChars
	...
	&& !( ( puncIndex == length-1         // and NOT over a dot that might
		|| !has_alnum( seq, puncIndex+1 ) )     // be a sentence-ender
	      ...
	      ( nextWrdStr == "</s>" ||
		nextWrdStr == "</S>"
		|| nextWrdStr.length() > 0 && nextWrdStr[0] == '(' ) ) )
    {
	ECString sseq(seq, puncIndex+1, length - puncIndex -1 );
...
/** In the raw text, the closing dot of a "stateLike" abbreviation may serve
    ...
    /**
     * MS: replaced ECString with StringWithOffsets
     *     following code accesses the StringWithOffset's surfaceForm_ 
     *     data member 
     */
								...
								// page FIVE
	{
		...
		{                                       //   ...
		    ...
		    savedWrdStr += "." ;
		    savedWrdStr += seq.substr(index+1,length-index-1);//side the
		    
		    savedWrd_ = StringWithOffsets( savedWrdStr, 
					   origStartOffset + puncIndex, 
					   origEndOffset );

		    StringWithOffsets retWord ( seq.substr( 0, puncIndex ),
						origStartOffset, 
						origStartOffset + puncIndex
						);

		    return retWord; // seq.substr(0,puncIndex);             // close-paren
		} else {              
		  // ...  unless
		  savedWrd_ = StringWithOffsets( seq.substr(puncIndex,length-puncIndex),
						 origStartOffset,
						 origStartOffset + puncIndex 
						 ); // close-paren
		...
		&& ( seq[puncIndex] == '"' || seq[puncIndex] == '\''
	      ...
	      {
	        savedWrdStr = ".";
		savedWrdStr += seq.substr(puncIndex,length-puncIndex);// add dot before
		...
		return retWord;             //  quote or
	      }                                                //  close-paren
	    
	    if( puncIndex == length )
	      {				     // If != length, there were unex-
		savedWrd_ = StringWithOffsets( ".", origEndOffset, origEndOffset );	     //  pected trailing characters,
		StringWithOffsets retWord( seq, origStartOffset, origEndOffset );
		return retWord;		     //  and we won't add a dot.
	      }
	}
	...
	savedWrd_ = StringWithOffsets( seq.substr(puncIndex,length-puncIndex),
				       origStartOffset + puncIndex,
				       origEndOffset 
				       ); // default: don't add dot

	return StringWithOffsets( seq.substr( 0,puncIndex ),
				  origStartOffset,
				  origStartOffset + puncIndex );
	...
	{                             // trailing-or-internal-apostrophe cases
		...
		if( length > 4 )
		{
		  if( seq.substr( 0,5 ) == "can't" || seq.substr( 0,5 ) == "Can't" )
		    {   savedWrd_ = StringWithOffsets( seq.substr( 2,length-2 ),
						       origStartOffset + 2,
						       origEndOffset
						       );

			string firstStr( seq.substr( 0,2 ) );
			firstStr += "n";

		        StringWithOffsets retv( firstStr,
						origStartOffset,
						origStartOffset + 2 
						);
                	return retv;
		    ...
		    {	
		      ...
		      string retv =  seq.substr( 0,1 );
		      retv += "ill";
		      return StringWithOffsets( retv,
						origStartOffset,
						origStartOffset + 2
						);
		    }
		    if( seq.substr( 0,5 ) == "ain't" || seq.substr( 0,5 ) == "Ain't" )
		      {	savedWrd_ = StringWithOffsets( seq.substr( 2,length-2 ),
                	...
                	return StringWithOffsets( "IS",
						  origStartOffset,
						  origStartOffset + 2
						  );       // caps as warning: "am"|"are"|"is"
		      }
		}
                                                                 // page SIX

		if( puncIndex > 1 )          // split  ...n't...  BEFORE the n
		  {   savedWrd_ = StringWithOffsets( seq.substr( puncIndex-1,length-puncIndex+1 ),
						     origStartOffset + puncIndex - 1,
						     origEndOffset 
						     );
		    
		    return StringWithOffsets( seq.substr( 0, puncIndex-1 ),
					      origStartOffset,
					      origStartOffset + puncIndex -1 
					      );
		  }
		// now puncIndex must == 1 and
		savedWrd_ = StringWithOffsets( seq.substr( 3,length-3 ),
					       origStartOffset + 3,
					       origEndOffset
					       );
		// length must be >= 3, so we
		return StringWithOffsets( seq.substr( 0,3 ),
					  origStartOffset,
					  origStartOffset + 3
					  );      // split  n't...  AFTER the t
	    }                                           // end ...n't... cases
		...
		return StringWithOffsets( seq.substr( 0,2 ),
					  origStartOffset,
					  origStartOffset + 2
					  );
	    }
	    ...
	    if( puncIndex == 4 && seq[3] == 'l'         // "of the" (Italian): 
                ...
                && seq[1] == 'e' && seq[2] == 'l' )
	      {   savedWrd_ = StringWithOffsets( seq.substr( 5,length-5 ),
						 origStartOffset + 5, 
						 origEndOffset
						 );
		
		return StringWithOffsets( seq.substr( 0,5 ),
					  origStartOffset,
					  origStartOffset + 5
					  );
	      }
	...
	savedWrd_ =
          StringWithOffsets( seq.substr(puncIndex,length-puncIndex),
			     origStartOffset + puncIndex,
			     origEndOffset
			     );    // default: split before puncChar
    ...
    if( seq[0] == '\"' )
      {
	savedWrd_ = StringWithOffsets( seq.substr( 1,length-1 ),
				       origStartOffset + 1,
				       origEndOffset 
				       );
      ...
      if( has_alnum( seq,0 )
	  || seq == "\"." && nextWrd_.surfaceForm_ == "." )   //eg  he said, ". . . Yes!"
	quotStr = "``";
      else
	quotStr = "''";
      
      return StringWithOffsets( quotStr,
				origStartOffset,
				origStartOffset + 1
				);  
      }
    ...
    if( length == 1 )                                     // isolated puncChar
      {   
	savedWrd_ = StringWithOffsets();
	return StringWithOffsets( seq, origStartOffset, origEndOffset );
      }
	...
	if( length > 3
	    ...
	    && ( seq[3] == 's' || seq[3] == 'l' ) )
	  {
	    savedWrd_ = StringWithOffsets( seq.substr( 4,length-4 ),
					   origStartOffset + 4,
					   origEndOffset
					   );
	    
	    return StringWithOffsets( seq.substr( 0,4 ),
				      origStartOffset,
				      origStartOffset + 4
				      );
	}
	...
	savedWrd_ = StringWithOffsets( seq.substr( lastSame,length-lastSame ),
				       origStartOffset + lastSame,
				       origEndOffset
				       );
	
	return StringWithOffsets( seq.substr( 0, lastSame ),
				  origStartOffset,
				  origStartOffset + lastSame
				  );
    }
    ...
    savedWrd_ = StringWithOffsets( seq.substr( 1,length-1 ),
				   origStartOffset + 1,
				   origEndOffset
				   );            // default treatment: return the
    
    return StringWithOffsets( seq.substr( 0,1 ),
			      origStartOffset,
			      origStartOffset + 1
			      );                        //  splitter puncChar as a Wrd
    }
...
//Added functions from here on

void ewDciTokStrm::advanceToNextWord() {
  	char c = istr_.peek();
...
void ewDciTokStrm::advanceSingleSpace() {
  	char c = istr_.peek();
...
::getNextWordFrom( istream & in,
	...
	bool isWord = false;
  	...
  	int beginOffset = currentCharOffset_;
  	int endOffset = beginOffset;

  	// skip leading whitespace, but increment character position
  	...
  	if ( in ) {
    		...
    		str.surfaceForm_ = word;
    		str.startOffset_ = beginOffset;
    		str.endOffset_ = beginOffset + word.length();
    		currentCharOffset_ = str.endOffset_ + 1;  // because we grabbed the next character

//     cerr << "## word: " << word << "; begin: " << beginOffset << "; end: " << ( str.endOffset_ + 1 ) << endl;
  	}
...
void ewDciTokStrm::resetCharOffset() {	// occurs when current word is leading matter, e.g. <s>

  	nextWrd_.startOffset_ = 0;
  	nextWrd_.endOffset_ = nextWrd_.surfaceForm_.length();

  	currentCharOffset_ = nextWrd_.endOffset_ + 1; 
