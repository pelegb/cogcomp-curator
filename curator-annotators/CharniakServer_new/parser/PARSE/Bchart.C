...
//   assert(len <= MAXSENTLEN);
  ...
  for(i = 0 ; i < len ; i++)
    {
      ECString wl = langAwareToLower(sentence[i].lexeme().surfaceForm_.c_str(), temp);
      int val = wtoInt(wl);
...
#ifdef DEBUG
  cerr << "## checking alreadyPoppedNum.." << endl;
#endif

	if ( alreadyPoppedNum >= 450000 )
	  throw CharniakException( "invalid value alreadyPoppedNum >= 450000." );
...
#ifdef DEBUG
  cerr << "## checking fh.cb..." << endl;
#endif

	if ( fh.cb == NULL)
	  throw CharniakException( "null fh.cb in Bchart::meEdgeProb" );
...
//  assert(fh.cb);
  ...
  cerr << "## checking fh.cb..." << endl;
	...
	if ( fh.cb == NULL)
	  throw CharniakException( "null fh.cb in Bchart::meFHProb" );
