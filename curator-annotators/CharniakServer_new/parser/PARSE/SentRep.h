...
public:
	...
	SentRep( vector< StringWithOffsets > & tokens_ );	//This is added constructor
	...
	/**
	   * if instantiated from vector of StringWithOffsets, 
	   *   there may be tokens that would otherwise have been 
	   *   replaced by ewTokStrm; so modify the strings to be
	   *   compatible.
	   */

	  StringWithOffsets normalizeToken( const StringWithOffsets str_, 
					    int & num_, 
					    const bool isLast_ 
					    );

	  string normalizeStringToken( const string str_, 
				       int & num_, 
				       const bool isLast_ );
