...
my $MAIN_CLASS="edu.illinois.cs.cogcomp.annotation.server.IllinoisCoreferenceServer";
...
my $LIB_CLASSPATH ="$SCRIPTS_HOME:$LIBDIR/CoNLLCoref-1.0.jar";
...
$LIB_CLASSPATH .= ":$LIBDIR/stanford-corenlp-2011-06-19.jar";
$LIB_CLASSPATH .= ":$LIBDIR/LBJLibrary-2.8.2.jar";
$LIB_CLASSPATH .= ":$LIBDIR/LBJ-2.8.2.jar";
...
croak "Usage: $0 IDENTIFIER PORT" unless @ARGV == 2;


my $ID = shift @ARGV; 
my $PORT = shift @ARGV;

my $CMD="nice $RUNIT java -cp $CLASSPATH -Dhome=$CURATOR_BASE -Xmx4G $MAIN_CLASS -p $PORT $ID";
