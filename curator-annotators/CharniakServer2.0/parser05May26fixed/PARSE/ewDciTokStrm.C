...
{                                            //  string if stream runs dry.
  ...
  for( ; ; ) {
    ...
    getNextWordFrom( ( useCin ? cin : istr_ ),
		     nextWord
		     );
...
// will be empty only if the input stream has run dry.
{
  ...
  while( savedWrd_.surfaceForm_ == "<s>"               // Starting in 1989, lines brack-
	 &&  nextWrd_.surfaceForm_ == "@" )            //  eted as sentences but with an
    {                                       //   @  following the  <s>  hold
      //cerr << "before @ flush" << endl;				      
      savedWrd_ = flush_to_sentence();    //  data arranged as charts or
      //cerr << "aft @ flush" << endl;				      
      
      getNextWordFrom( istr_, 
		       nextWrd_
		       );
    }
  if(docEnd)
  ...
  if(retWrdStr == "(") retWrdStr = "-LRB-";
  else if(retWrdStr == ")") retWrdStr = "-RRB-";
  else if(retWrdStr == "{") retWrdStr = "-LCB-";
  else if(retWrdStr == "}") retWrdStr = "-RCB-";
  else if(retWrdStr == "[") retWrdStr = "-LSB-";
  else if(retWrdStr == "]") retWrdStr = "-RSB-";
