...
{
  bool ans;
  const char* wrd = sentence_[pos].lexeme().surfaceForm_.c_str();
  if(pos > endPos) ans = 0;
  ...
  //else if(ChartBase::finalPunc(wrd) || !strcmp(wrd, ";")) ans = 1;
    ...
    {
      if(sentence_[pos+1].lexeme().surfaceForm_ == "''")
...
addtochart(const Term* trm)
{
  if(numItemsToDelete >= itemsToDeletesize)
    {
      Item* dummy = new Item(trm, 0, 0, numParses);
...
ChartBase::
ChartBase(SentRep & sentence, const int numParses_ )
: crossEntropy_(0.0L), 
  wrd_count_(0),
  ruleiCounts_(0),
  popedEdgeCount_(0),
  ...
  numParses( numParses_ )
{
    ...
    if(wrd_count_ > 0) endwrd = sentence_[wrd_count_-1].lexeme().surfaceForm_.c_str();
    if(endwrd  && finalPunc(endwrd)) endPos = wrd_count_-1;
    else if(wrd_count_ > 2)
      {
	endwrd = sentence[wrd_count_-2].lexeme().surfaceForm_.c_str();
	if(finalPunc(endwrd)) endPos = wrd_count_-2;
	else
	  {
	    endwrd = sentence[wrd_count_-3].lexeme().surfaceForm_.c_str();
...
// 		  assert(tempPos < 400);
		  if ( tempPos >= 400 )
		    throw CharniakException( "invalid value tempPos >= 400" );
