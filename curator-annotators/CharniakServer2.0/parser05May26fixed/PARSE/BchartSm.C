...
// 	  assert(pUstream);
	  if ( NULL == pUstream )
	    throw CharniakException( "NULL input stream pUstream." );
	}
      for( i = 0 ; i <  nm ; i++ )
	{
	  int t;
...
//   assert(resultsStream);
      if ( NULL == resultsStream )
	throw CharniakException( "NULL output stream resultsStream." );
...
void
Bchart::
readTermProbs( ECString& path, const int numParses_ )
{
  int i,j;
  for(i = 0 ; i < MAXSENTLEN ; i++) 
    stops[i] = new Item( Term::stopTerm, i, i, numParses_ );
...
//   assert(pTstream);
  ...
  if ( NULL == pTstream )
    throw CharniakException( "NULL input stream pTstream." );

  ifstream ppTstream(ppTstring.c_str());
//   assert(ppTstream);
  if ( NULL == ppTstream )
    throw CharniakException( "NULL input stream ppTstream." );
...
//       assert(i < numpT);
      ...
      if ( i >= numpT )
	throw CharniakException( "invalid index i >= numpT." );
...
//       assert(t < MAXNUMNTTS)
      if ( t >= MAXNUMNTTS )
	throw CharniakException( "invalid index t >= MAXNMTTS." );
...
//   assert(wlistStream);
  ...
  if ( NULL == wlistStream )
    throw CharniakException( "NULL input stream wlistStream." );
...
// 	  assert(trm);
	  ...
	  if ( NULL == trm )
	    throw CharniakException( "NULL trm pointer." );
...
//   assert(wrd_count_ < 1000);
  ...
  if ( wrd_count_ >= 1000 )
    throw CharniakException( "invalid value wrd_count_ >= 1000" );
...
// 	  assert(prb >= 0);
	  ...
	  if ( prb < 0 )
	    throw CharniakException( "invalid value prb < 0." );
...
// 	      assert(pk > 0);
	      ...
	      if ( smb <= 0 )
		throw CharniakException( "invalid value smb <= 0." );
	      if ( pk <= 0 )
		throw CharniakException( "invalid value pk <= 0." );
...
//       assert(pw0n > 0);
      ...
      if ( pw0n <= 0 )
	throw CharniakException( "invalid value pwOn <= 0." );
...
// 	  assert( prob > 0);
	  if ( prob <= 0 )
	    throw CharniakException( "invalid value prob <= 0." );
...
//   assert(!ans.empty());
  ...
  if ( ans.empty() )
    throw CharniakException( "empty answer." );
...
//       assert(ans >= 0);
      if ( ans < 0 )
	throw CharniakException( "invalid value ans < 0." );
...
pCapgt(const Wrd* shU, int t)
{
  ...
  const ECString& lex0 = sentence_[0].lexeme().surfaceForm_;
  if(word_num == 0) return 1;
  else if(word_num == 1 &&
	  (lex0 == "``" || lex0 == "-LCB-" || lex0 == "-LRB-"))
    return 1;
  //cerr << "pCapgt = " << pcap << endl;
  if(shU->lexeme().surfaceForm_.length() < 2) return 1;  //ignore words of length 1;
  char temp[128];
  ECString sh(toLower( ( shU->lexeme() ).surfaceForm_.c_str(),temp));
  bool cap = false;
  /* if all caps, ignore capitalization evidence */
  if( ( shU->lexeme() ).surfaceForm_[0] != sh[0] && 
      ( shU->lexeme() ).surfaceForm_[1] != sh[1] ) 
    return 1;
...
//   assert(wordInt >= 0);
  if ( wordInt < 0 )
    throw CharniakException( "invalid value wordInt < 0." );

  FeatureTree* strt = FeatureTree::roots(HCALC);

  //   assert(strt);
  if ( NULL == strt )
    throw CharniakException( "NULL strt pointer." );
...
//   assert(ans >= 0);
  ...
  if(ans == 0) return 0;
  double phyp = 1;
  if(Term::Language != "Ch") phyp = pHypgt( ( shU->lexeme() ).surfaceForm_,t);
  ans *= phyp;
...
//   assert(whichFt < MAXNUMFS);
  if ( whichInt >= Feature::numCalcs )
    throw CharniakException( "invalid value whichInt > Feature::numCalcs" );
  if ( whichFt >= MAXNUMFS )
    throw CharniakException( "invalid value whichFt >= MAXNUMFS." );
...
computeMerit(Edge* edge, int whichDist)
{
  ...
  if(denom <= 0)
    {
      cerr << "ERROR: invalid denom prob value " <<  denomPos << " for edge " 
	   << *edge << endl;
//       assert(denom > 0);
      if ( denom <= 0 )
	throw CharniakException( "invalid value denom <= 0." );
    }
