  ...
  for(int which = 0 ; which < Feature::numCalcs ; which++)
    {
      ...
      if ( NULL == fts ) 
	throw CharniakException( "null filestream." );
...
//   assert(s);
  ...
  if ( NULL == s )
    throw CharniakException( "null item pointer." );
...
//   assert(atp.probs[0] >= 0);
  ...
  if ( atp.probs[0] < 0 )  
    throw CharniakException( "invalid probability score (<0)." );

//   assert(atp.trees[0].subTrees.front()->trm < 400);

  if ( atp.trees[0].subTrees.front()->trm >= 400 )
      throw CharniakException( "subtrees.front()->trm >= 400." );
...
bestParse(Item* itm, FullHist* h, AnsTree* cat, AnsTree* gcat, int cdir)
{
  ...
  AnsTreeMap atm;
  ECString bestW;
  CombineBests combBests( m_numParses );
  for( ; pi != pm.end() ; pi++ )
    {
      int posInt = (*pi).first;
      if(printDebug() > 16)
      ...
      for( ;hi != hm.end();hi++)
	{
	  ...
	  ECString subh = subhw.lexeme().surfaceForm_;
	  if(printDebug() > 16)
	    {
	      prDp();
...
// 		  assert(hprob >=0);
		  if ( hprob < 0 )
		    throw CharniakException( "invalid probability for hprob (<0)." );
		}
	    }
		   ...
		   FullHist* h, ItmGHeadInfo& ighInfo, AnsTree* cat, AnsTree* gcat)
{
  if( printDebug() > 1000 ) {
    m_depth++;
    cerr << "MeChart::bestParseGivenHead(): (depth " << m_depth << "):" 
	 << endl;
    cerr << "posInt: " << posInt << ", wd: " << wd << "; item: " 
	 << *itm << "; fullHist: " << *h << "; ighInfo: " 
	 << ighInfo.first.size() << " edges, " << ighInfo.second.size()
	 << " answer trees; gcat: " << gcat << endl << endl;
  }
  ...
  if( printDebug() > 1000 ){
    cerr << "## calling recordedBPGH()..." << endl;
  }

  AnsTreeStr& atp = recordedBPGH(itm, atm, h);

  if( printDebug() > 1000 ) {
    cerr << "## back from recordedBPGH()..." << endl;
  }
  ...
  CombineBestsGh combBests( m_numParses );
  double bestP = 0;
  double sumP = 0;
  AnsTree bestAT;
  ...
  for( ; ei != es.end() ; ei++)
      ...
      if(printDebug() > 20)
	{
      ...
      gcurAnswerTree=NULL;
      CombineBestsT combBestsT(trm, prob*edgePg, m_numParses);
...
recordedBPGH(Item* itm, AnsTreeMap& atm, FullHist* h)
{
  ...
  CntxArray ca(subfv);
  return atpFind(ca, atm, m_numParses);
}
...
// 	      assert(histPt);
	      if ( NULL == histPt ) 
		    throw CharniakException( "Null pointer." );
...
{
  if ( printDebug() > 1000 ) 
    cerr << "start getHt" << endl;

  int i;
  for(i = 1 ; i < MAXNUMFS ; i++) subfVals[i] = -1;

  if(printDebug() > 1000 ) {
    cerr << "## number of features/ iterations: " 
	 << Feature::total[whichTree] << "'..." << endl;
  }
  ...
  for(i = 1 ; i <= Feature::total[whichTree] ; i++)
    {
      ...
      if(printDebug() > 1000 ) {
	cerr << "## iteration " << i << "... calling Feature::fromInt("
	     << i << ", " << whichTree << ")..." << endl;
      }
      ...
      if(printDebug() > 1000 ) {
	cerr << "## back from Feature::fromInt()..." << endl;
      }

      int sfInt = ft->subFeat;
      if(printDebug() > 1000 ) {
	cerr << "## iteration " << i << "... calling SubFeature::fromInt("
	     << sfInt << ", " << whichTree << ")..." << endl;
      }
      SubFeature* sf = SubFeature::fromInt(sfInt, whichTree);

      if(printDebug() > 1000 ) {
	cerr << "## back from SubFeature::fromInt()... feature fun # is: "
	     << sf->usf << "..." << endl;
      }
      ...
      if ( printDebug() > 1000 ) 
	cerr << "## getHt: val is '" << val << "'... adding to subfVals[" << sfInt << "]..." << endl;

      subfVals[sfInt] = val;

      if ( printDebug() > 1000 ) 
	cerr << "## getHt: val is '" << val << "'... loop ended..." << endl;
    }
