...
{  
  ...
  if ( args.isset( 'P' ) ) // num of best parses to return 
    {
      numParses_ = atoi( args.value( 'P' ).c_str() );

      if ( numParses_ <= 0 ) {
	stringstream errStrm;
	errStrm << "ERROR: Params::init(): non-integer value '"
		<< args.value( 'P' ) << "' for number of parses." 
		<< endl;
	cerr << errStrm.str();
	exit( -1 );
      }
