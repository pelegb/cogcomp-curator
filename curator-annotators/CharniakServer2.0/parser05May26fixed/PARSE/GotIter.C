...
//       assert(i < 400);
      ...
      if ( i >= 400 )
	throw CharniakException( "invalid index ( i >= 400 )." );
...
next(Item*& itm)
{
  ...
  if ( pos_ >= 400 )
	throw CharniakException( "invalid index ( pos_ >= 400 )." );
  ...
  for( ; eiter != itm->needme().end() ; eiter++)
    {
      ...
      if ( stackptr >= 64000 )
	throw CharniakException( "invalid stackptr value ( >= 64000)." );
...
next(Item*& itm, int& dir)
{
  ...
  if ( pos_ >= 64 )
	throw CharniakException( "invalid index ( pos_ >= 64 )." );
...
//       assert(i < 64);
      ...
      if ( i >= 64 )
	throw CharniakException( "invalid index ( i >= 64 )." );
...
//           assert(i > 0);
	  if ( i <= 0 )
	    throw CharniakException( "invalid index ( i <= 0 )." );
