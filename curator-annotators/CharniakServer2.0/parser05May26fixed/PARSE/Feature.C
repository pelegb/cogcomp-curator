...
//       assert(conditioned == "m");
      if ( conditioned != "m" )
	throw CharniakException( "incorrect value: conditioned != m." );
...
init(ECString& path, ECString& conditioned)
{
  ...
  if ( NULL == dataStrm )
    throw CharniakException( "null dataStrm." );
...
//       assert(SubFeature::fromInt(n, whichInt));
      if ( !SubFeature::fromInt(n, whichInt) )
	throw CharniakException( "failed: SubFeature::fromInt(n, whichInt)." );
    }
...
//       assert(fts);
      ...
      if ( NULL == fts )
	throw CharniakException( "null input stream fts." );
...
//   assert(res);
  ...
  if ( NULL == res )
    throw CharniakException( "null output stream res." );
...
//   assert(posftTree);
  if ( NULL == posftTree )
    throw CharniakException( "null pointer posftTree." );
  if(!posftTree->left)
    {
