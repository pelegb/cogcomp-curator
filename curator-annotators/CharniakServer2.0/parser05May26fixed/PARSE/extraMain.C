...
void
generalInit( ECString path, const int numParses_ )
{
  ...
  ur->readData(path); 
  Bchart::unitRules = ur;
  Bchart::readTermProbs(path, numParses_ );
  MeChart::init(path);
  Bchart::setPosStarts();
...
//   assert(trmInt < 400);
  ...
  if ( trmInt >= 400 ) 
    throw CharniakException( "at->trm >= 400." );
  ...
  if(trm && trm->terminal_p())
    {
      if(sr.length() > 0) {
	wrdString = sr[pos].lexeme().surfaceForm_;
	startOffset = sr[pos].lexeme().startOffset_;
	endOffset = sr[pos].lexeme().endOffset_;
      }
      else 
	wrdString = Bchart::intToW(at->wrd);
      ...
      if ( NULL == at )
	{
	  ...
	  errStrm << "ERROR: extraMain::inpuTreeFromAnsTree(): " 
		  << "null at pointer." << endl;
      ...
      for( ; ati != at->subTrees.end() ; ati++)
	{
	  ...
	  int childStartOffset = sit->startOffset();
	  if ( childStartOffset < startOffset ) 
	    startOffset = childStartOffset;

	  int childEndOffset = sit->endOffset();
	  if ( childEndOffset > endOffset ) 
  ...
  ans = new InputTree(begn, pos, wrdString, trmString, ntString,
		      subtrs, startOffset, endOffset, NULL, NULL);
