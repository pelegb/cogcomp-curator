...
//   assert(size > 0);
  ...
  if ( size <= 0 )
    throw CharniakException( "invalid value 'size' <= 0." );
  ...
  if ( bestPos < 0 )
    throw CharniakException( "invalid value 'bestPos' < 0." );
  ...
  for(int i = 0 ; i < numParses ; i++)
    {
      ...
      if(atsp < 0) break;
      double p1 = prob * atsp;
      if(atq.size == numParses && p1 < atq.worstP) break;
...
//       assert(ats.trees[i].trm >= 0);
      if ( ats.trees[i].trm < 0 )
	throw CharniakException( "invalid value 'ats.tree[i].trm' < 0." );
  ...
  for(int i = 0 ; i < cbt.curAtq().size ; i++)
    {
      ...
      if(atq.size < numParses || p1 > atq.worstP)
...
CombineBestsT::
CombineBestsT(const Term* tm, double prb, const int numParses_ ) : 
  rprob(prb), 
  trm(tm), 
  whichIsCur(false), 
...
extendTrees(AnsTreeStr& ats,int dir)
{
  ...
  for(int i = 0 ; i < numParses ; i++)
      ...
      for(int j = 0 ; j < pastAtq().size ; j++)
	{
	  ...
	  if(curAtq().size == numParses && p2 <= curAtq().worstP) continue;
	  addedNew = true;
