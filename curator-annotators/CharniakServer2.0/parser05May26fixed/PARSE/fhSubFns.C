...
int toBe( const StringWithOffsets & parw )
{
#ifdef DEBUG 
    cerr << "## toBe(): arg is " << endl;
    parw.show( cerr );
    cerr << "..." << endl;
#endif

  string str = parw.surfaceForm_;
...
#ifdef DEBUG 
    cerr << "## fh_parent_pos: par is " << par << "..." << endl;
    if ( !par ) 
      cerr << "## returning '" << stopint << "'..." << endl;
#endif
...
#ifdef DEBUG 
    cerr << "## fh_parent_pos: ans is '" << ans << "'..." << endl;
#endif

  if(ans < 2) {
#ifdef DEBUG 
      cerr << "## par hd is " << par->hd << endl;
#endif    
    if (toBe(par->hd->lexeme())) return 48;
  }
...
//       assert(st);
      if ( NULL == st )
	throw CharniakException( "null pointer 'st'." );
      return st->term;
    }
...
//   assert(ans >= -1);
  if ( ans < -1 )
    throw CharniakException( "invalid value: ans < -1." );
...
//   if ( ans < -1 )
    throw CharniakException( "invalid location in code... shouldn't get here." );
  return -1;
}
...
//   assert(par->e);
  if ( !par->e )
    throw CharniakException( "invalid value: par->e" );
...
//       assert(loc < sz);
      if ( loc >= sz )
	throw CharniakException( "invalid value: loc >= sz." );

      FullHist* nxt = fhs[loc];

      //       assert(nxt);
      if ( NULL == nxt )
	throw CharniakException( "null pointer 'nxt'." );
...
//   assert(lrgi);
  if ( NULL == lrgi )
    throw CharniakException( "null pointer 'lrgi'." );
  if(m >= lrgi->size()) return stopTermInt;
  Item* got = lrgi->index(m);
//   assert(got);
  if ( NULL == got )
    throw CharniakException( "null pointer 'got'." );
