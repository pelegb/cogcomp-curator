...
Bchart::
Bchart( SentRep & sentence, const int numParses_ )
  : ChartBase( sentence, numParses_ ),
...
#ifdef DEBUG
  cerr << "## checked sentence length..." << endl;
#endif

  if ( len > MAXSENTLEN )
    throw CharniakException( "invalid value len > MAXSENTLEN." );

  char temp[250];
  for(i = 0 ; i < len ; i++)
    {
      ECString wl = toLower(sentence[i].lexeme().surfaceForm_.c_str(), temp);
      int val = wtoInt(wl);
      
#ifdef DEBUG 
      cerr << "## Bchart: sentence[" << i << "] string is '"
	   << wl << "; value is: " << val << "..." << endl;
#endif
...
#ifdef DEBUG
  cerr << "## checking loc value length..." << endl;
#endif

  if ( !( loc >= 0 && loc < MAXSENTLEN ) )
    throw CharniakException( "invalid value loc (<0 or >MAXSENTLEN)." );
...
//   assert(i >= 0 && i < MAXSENTLEN && start >= 0 && start < MAXSENTLEN);
...
#ifdef DEBUG
  cerr << "## checking indexes start, i within sentence length..." << endl;
#endif

  if ( !( i >= 0 && i < MAXSENTLEN && start >= 0 && start < MAXSENTLEN ) )
    throw CharniakException( "invalid value i or start." );
  ...
  for( ; regsiter != regs[i][start].end() ; regsiter++)
	...
	{
	  stringstream errStrm;
	  errStrm << "Bchart::already_there_extention() : "
		  << e.what()  << endl;
...
add_starter_edges(Item* itm)
{
  ...
  for(i = 0 ; ; i++)
	...
	{
	  stringstream errStrm;
	  errStrm << "Bchart::add_starter_edges() : "
		  << e.what()  << endl;
...
extend_rule(Edge* edge, Item * item, int right)
{
	...
	try{	
	  ...
	  {
	    stringstream errStrm;
	    errStrm << "Bchart::extend_rule() (edge->start case): "
		    <<  e.what()  << endl;
	...
	try{
	  ...
	  {
	    stringstream errStrm;
	    errStrm << "Bchart::extend_rule() ('right' case): "
		    << e.what()  << endl;
	...
	try{
	  ...
	  {
	    stringstream errStrm;
	    errStrm << "Bchart::extend_rule() ('left' case): "
		    << e.what()  << endl;
...
#ifdef DEBUG
  cerr << "## checking alreadyPopedNum.." << endl;
#endif

	if ( alreadyPopedNum >= 450000 )
	  throw CharniakException( "invalid value alreadyPopedNum >= 450000." );
...
#ifdef DEBUG
      cerr << "## checkinge newEdge->finishedParent()..." << endl;
#endif

      if ( newEdge->finishedParent() != regi )
	throw CharniakException( "invalid value newEdge->finishedParent() != regi." );
  }
...
#ifdef DEBUG
  cerr << "## checking indexes i, j less than limits..." << endl;
#endif

  if ( i >= MAXNUMNTTS )
    throw CharniakException( "invalid value i > MAXNUMNTTS." );
  if ( j >= MAXNUMNTS )
    throw CharniakException( "invalid value j > MAXNUMNTS." );
...
#ifdef DEBUG
  cerr << "## checking index j..." << endl;
#endif

	  if ( numFor[j] >= MAXNUMNTTS )
	    throw CharniakException( "invalid value numFor[j] > MAXNUMNTTS." );
...
#ifdef DEBUG
  cerr << "## checking ginfo[0]..." << endl;
#endif

  if ( !ginfo[0] )
    throw CharniakException( "null or zero value ginfo[0]." );
...
// 	  assert(histPt);
#ifdef DEBUG
  cerr << "## checking histPt..." << endl;
#endif

	  if ( NULL == histPt )
	    throw CharniakException( "null pointer histPt." );
...
#ifdef DEBUG
  cerr << "## checking index i >= 2..." << endl;
#endif

	  if ( i < 2 )
	    throw CharniakException( "invalid value: i < 2" );
