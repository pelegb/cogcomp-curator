...
logger.info(archiveClassname);
logger.info(archivefn);
    ...
    private Record getRecordUsing(String input, String methodName){
        Record record = null;
        ...
        try {
            transport.open();
            record = (Record) client.getClass().getMethod(methodName).invoke(input);
            transport.close();
        } catch (TException e) {
            if (transport.isOpen())
                transport.close();
	...
	private Record getRecord(String text, boolean whitespaced)
		...
		logger.debug( "Archive responded to request. Looking at response." );
...
//				}
			    ...
			    record = getRecordUsing(text, "getRecord");
...
//				}
//			}
            record = getRecordUsing(identifier,"getRecordById");
		}
		if (record == null) {
			throw new AnnotationFailedException(
...
// 	}
	...
	public void performAnnotation(Record record, String view_name,
		...
		if (!slave) {
			...
			try {
				...
				logger.warn("Unhandled TException (probably a problem in the underlying annotator): message '" + e.getMessage() + "'." );
	...
	private void transform(Record record, String view_name, Object client)
		...
		if (client instanceof Labeler.Iface) {
			...
			logger.debug( "received labeling with '" + labeling.getLabels().size() + "' spans for view '" + view_name + "'." );
